...
if(!fs.existsSync(path.join(root,'public/favicon.png')))failures.push('current brand icon is missing: public/favicon.png');
if(fs.existsSync(path.join(root,'public/favicon.svg')))failures.push('legacy brand icon file remains: public/favicon.svg');
if(index.includes('favicon.svg')||index.includes('<svg'))failures.push('legacy brand icon reference remains in public/index.html');
...
if(/mcp_servers\./.test(officialService))failures.push('persistent official service must not inject a project-defined MCP server');
