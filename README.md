...
The pinned Codex baseline exposes official pagination, `thread/items/list`, and `thread/searchOccurrences`. Quick view is capped at the 10 most recent turns: it uses one official `itemsView: notLoaded` page to bound startup and does not expose an older-page control. Full-history mode is the only human-facing page-by-page flow: it shows the recent page structure immediately, keeps the page marker and older-page control sticky at the top, and loads only the pages needed by an explicit load action or full-history search. Visible conversation/work-process content hydrates per turn through the official `thread/items/list` endpoint when a turn approaches the viewport or the user explicitly asks for it. Search uses the official occurrence index, displays its official excerpts, and also checks already-rendered work-process text, so the UI does not invent a private ChatGPT endpoint. Each history page is requested separately, with a smaller-page retry if a large page crosses the transport guard, so the aggregate conversation is not sent as one 128 MiB JSONL line. Pagination removes the aggregate-session startup limit; a single unusually large official turn can still hit the transport guard and must be compacted upstream.
