# Architecture

## Control plane

```text
Browser
  ├─ native Codex conversation UI
  ├─ Official APIs schema drawer
  └─ MCP Apps Host
       └─ opaque-origin Sandbox Proxy iframe
            └─ untrusted MCP App srcdoc iframe
           HTTP/SSE
              |
              v
Node Web Gateway
  ├─ auth / same-origin / access-profile gate
  ├─ official JSON+TS protocol registry
  ├─ stable + experimental disposition seal
  ├─ currentTime/read host responder
  └─ Dynamic Tool process host (optional, experimental)
              |
           official WebSocket (Linux service) / stdio JSONL (portable)
              v
       official codex app-server
              |
              v
       official Codex runtime
```

## Protocol authority

At startup `OfficialSchemaRegistry` runs the installed Codex binary's `app-server generate-json-schema` and `generate-ts`. The JSON wire set is authoritative for invocation. TypeScript provides an independent export-coverage check. Cached schemas are tied to exact Codex version, mode and digest.

## Browser event model

The gateway converts official server notifications and non-host-managed ServerRequests into bounded SSE events. Before specialized browser handling, every delivered schema-admitted notification enters the bounded Official Events observer; transport/process events remain visible there without polluting the conversation. Every SSE client has its own bounded backpressure queue and keep-alive heartbeat, so a slow mobile connection cannot evict or stall another Web client; reconnect is followed by an authoritative resync rather than replaying uncertain writes. The browser keeps live item state keyed by official item id; the gateway exposes a per-boot runtime identity through `/api/meta` and `connected`, and recovery reads the remembered thread through official `thread/read` before any explicit write can request `thread/resume`. Official runtime activity is tracked separately from local Web write ownership: every selected Web view may show the official Turn as running, while only the Web tab that owns the local turn marker can steer, stop, compact, or display live context usage. This preserves a single-writer boundary without adding a second desktop-ownership guard: an external official turn remains readable and is not implicitly resumed, while an actual active-writer response becomes a Web read-only state. The gateway’s single Web-write switch is enforced on every non-read browser RPC/notification/response, and the official App Server remains authoritative for concurrent writes. Linux deployment keeps the official App Server in a separate supervised WebSocket service; a gateway reconnect therefore does not own or terminate an in-flight official Turn. The UI groups non-message items into a compact work-process disclosure, routes an input during an active turn through official `turn/steer`, and surfaces official `thread/compacted` / `thread/compact/start` state without inventing a private conversation protocol. Official plan progress is rendered in a persistent bottom card whose mobile expanded/collapsed state survives live progress re-renders; desktop hover/focus and mobile tap use the same official plan data. Quick history reads one official `itemsView: notLoaded` page for the 10 newest Turns, then hydrates all ten Turns through official `thread/items/list` with bounded concurrency; complete history alone follows older cursors and hydrates older Turn items only when visible or explicitly requested. A mode switch establishes a new authoritative page boundary, so cached full items cannot strand quick-view placeholders; cancelled or superseded hydration returns to a retryable state. While a local Web Turn is active, a bounded official `thread/turns/list` read-only watchdog reconciles terminal state so a half-open SSE stream cannot leave a permanent running indicator. If the separately supervised official App Server itself is stopped or crashes, recovery never mislabels the terminated generation as still running.

The gateway writes no append-only conversation or runtime log files. Its only runtime-owned persistent data is the generated official schema cache; startup maintenance removes only stale, exact-name `.tmp`/`.bak` swap artifacts from that cache. Official Codex session/history storage remains owned by the official runtime and is intentionally not pruned by this project.

## MCP Apps

The capability is fixed during App Server initialization and flows downstream to MCP sessions. `mcpToolCall.appContext.resourceUri` (or the compatibility resource URI) selects the `ui://` resource. The host reads it through official `mcpServer/resource/read`, validates the MIME/profile and size, then sends raw HTML plus approved CSP/permissions to the outer Sandbox Proxy.

The outer proxy is a `data:` document, giving it an opaque origin different from the product page while still permitting the spec-required `allow-scripts allow-same-origin`. It creates the second iframe using the stable reference profile `allow-scripts allow-same-origin allow-forms`. That View can share the proxy's isolated origin, but it still cannot become same-origin with the product Host because the outer `data:` proxy has a distinct opaque origin. All View↔Host messages are JSON-RPC, origin/source-validated and size-bounded.

App server tool/resource calls are proxied back through official `mcpServer/*` App Server methods. Tool calls are scoped to the originating server and checked against `_meta.ui.visibility`.

## Dynamic Tools

Configured Dynamic Tool definitions are converted to the official canonical `thread/start.dynamicTools` shape only in experimental mode. When App Server sends `item/tool/call`, the gateway matches namespace+tool against the operator registry. Matching handlers are spawned directly with `shell:false`; arguments from the model are stdin JSON, never command-line shell text. Unconfigured or unmatched calls receive the official `-32601` response and never become fake approval UI.

## Trust boundaries

The Web token authenticates only this gateway. It is stripped from Codex and Dynamic Tool child environments. Codex owns its own account/config/session state. MCP App code is untrusted. Dynamic Tool executables are operator-trusted local programs but are resource-bounded by the host.
