...
test('composer keeps workspace selection in the primary new-thread flow and supports official image input', () => {
  ...
  assert.match(app, /sealedInvalidateTurnItemCache/);
  assert.match(app, /historyBypassItemCacheThread/);
  assert.match(app, /state\.renderedHistoryMode&&state\.renderedHistoryMode!==historyViewKey\(\)/);
  assert.match(app, /preserveOptimistic/);
