...
test('gateway rejects an unconfigured Dynamic Tool call without exposing a manual approval card',async(t)=>{
 ...
 const unsupported= (async()=>{try{const reader=events.body.getReader(),decoder=new TextDecoder();let buffer='';for(;;){const {done,value}=await reader.read();if(done)return null;buffer+=decoder.decode(value,{stream:true});const frames=buffer.split('\n\n');buffer=frames.pop()||'';for(const frame of frames){const line=frame.split('\n').find(x=>x.startsWith('data: '));if(!line)continue;const event=JSON.parse(line.slice(6));if(event.type==='serverRequestUnsupported')return event;}}}catch{return null;}})();
 const start=await fetch(`${url}/api/rpc`,{method:'POST',headers:{origin,cookie,'content-type':'application/json'},body:JSON.stringify({method:'thread/start',params:{cwd:dir}})});assert.equal(start.status,200,output);
 ...
 await new Promise(resolve=>setTimeout(resolve,80));controller.abort();assert.equal(await unsupported,null);
});
