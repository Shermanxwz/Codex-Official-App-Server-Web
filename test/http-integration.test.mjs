...
if(args[0]==='app-server'&&args[1]==='generate-json-schema'){
 ...
 const req={definitions:{ThreadListParams:{type:'object',properties:{limit:{type:'integer'}}}},oneOf:[{title:'Thread/listRequest',properties:{id:{},method:{enum:['thread/list']},params:{$ref:'#/definitions/ThreadListParams'}},required:['id','method','params']}]};
...
async function waitReady(url, child){
  for(let i=0;i<80;i++){
    ...
    try{const r=await fetch(`${url}/api/session`);if(r.ok)return;}catch{}
...
test('full HTTP gateway admits only methods exported by official schema', async(t)=>{
  ...
  const methods=await (await fetch(`${url}/api/methods`,{headers:{cookie}})).json(); assert.deepEqual(methods.requests.map(x=>x.method),['thread/list']);
