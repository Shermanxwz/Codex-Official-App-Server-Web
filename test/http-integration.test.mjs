...
if(args[0]==='app-server'&&args[1]==='generate-json-schema'){
 ...
 for(const [name,data] of [['ClientRequest.json',req],['ClientNotification.json',note],['ServerRequest.json',sreq],['ServerNotification.json',snote]]) fs.writeFileSync(path.join(out,name),JSON.stringify(data));
 process.exit(0);
}
if(args[0]==='app-server'&&args[1]==='generate-ts'){
 const out=args[args.indexOf('--out')+1]; fs.mkdirSync(out,{recursive:true});
 const files={
  'ClientRequest.ts':'export type ClientRequest = { "method": "initialize" } | { "method": "thread/list" };',
  'ClientNotification.ts':'export type ClientNotification = { "method": "client/ping" };',
  'ServerRequest.ts':'export type ServerRequest = { "method": "item/commandExecution/requestApproval" };',
  'ServerNotification.ts':'export type ServerNotification = { "method": "thread/started" };',
 };
...
async function waitReady(url, child){
  for(let i=0;i<80;i++){
    ...
    try{const r=await fetch(`${url}/readyz`);if(r.status===200)return;}catch{}
    await new Promise(r=>setTimeout(r,50));
  }
  throw new Error('server/app-server did not become ready');
}
...
test('full HTTP gateway admits only methods exported by official schema', async(t)=>{
  ...
  const child=spawn(process.execPath,['src/server.mjs'],{cwd:root,env:{...process.env,CWEB_CODEX_BIN:codex,CWEB_STATE_DIR:path.join(dir,'state'),CWEB_WORKSPACE:dir,CWEB_HOST:'127.0.0.1',CWEB_PORT:String(port),CWEB_REQUIRE_AUTH:'1',CWEB_TOKEN:token},stdio:['ignore','pipe','pipe']});
  ...
  const ready=await fetch(`${url}/readyz`); assert.equal(ready.status,200);
