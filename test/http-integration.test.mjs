...
if(args[0]==='app-server'&&args[1]==='generate-json-schema'){
 ...
 const req={definitions:{InitializeParams:{type:'object'},ThreadStartParams:{type:'object',properties:fields(['threadId','cwd','approvalPolicy','sandbox','permissions'])},ThreadResumeParams:{type:'object',properties:fields(['threadId','approvalPolicy','sandbox','permissions'])},ThreadSettingsUpdateParams:{type:'object',properties:fields(['threadId','approvalPolicy','sandboxPolicy','permissions','collaborationMode'])},TurnStartParams:{type:'object',properties:fields(['threadId','input','approvalPolicy','sandboxPolicy','permissions'])},CommandExecParams:{type:'object',properties:fields(['command','sandboxPolicy','permissionProfile'])}},oneOf:[variant('initialize','InitializeParams'),variant('thread/start','ThreadStartParams'),variant('thread/resume','ThreadResumeParams'),variant('thread/settings/update','ThreadSettingsUpdateParams'),variant('turn/start','TurnStartParams'),variant('command/exec','CommandExecParams')]};
 const empty={definitions:{},oneOf:[]};
 for(const [name,data] of [['ClientRequest.json',req],['ClientNotification.json',empty],['ServerRequest.json',empty],['ServerNotification.json',empty]])fs.writeFileSync(path.join(out,name),JSON.stringify(data));process.exit(0);
}
if(args[0]==='app-server'&&args[1]==='generate-ts'){
...
test('full HTTP gateway admits only methods exported by official schema', async(t)=>{
  ...
  const badParams=await fetch(`${url}/api/rpc`,{method:'POST',headers:{'content-type':'application/json',origin,cookie},body:JSON.stringify({method:'thread/list',params:[]})}); assert.equal(badParams.status,400); assert.equal((await badParams.json()).error,'INVALID_PARAMS_OBJECT');
  const badNotificationParams=await fetch(`${url}/api/notify`,{method:'POST',headers:{'content-type':'application/json',origin,cookie},body:JSON.stringify({method:'client/ping',params:[]})}); assert.equal(badNotificationParams.status,400); assert.equal((await badNotificationParams.json()).error,'INVALID_PARAMS_OBJECT');
  const badEnvelope=await fetch(`${url}/api/rpc`,{method:'POST',headers:{'content-type':'application/json',origin,cookie},body:JSON.stringify([])}); assert.equal(badEnvelope.status,400); assert.equal((await badEnvelope.json()).error,'INVALID_JSON_OBJECT');
  const badMethod=await fetch(`${url}/api/rpc`,{method:'POST',headers:{'content-type':'application/json',origin,cookie},body:JSON.stringify({method:'',params:{}})}); assert.equal(badMethod.status,400); assert.equal((await badMethod.json()).error,'INVALID_METHOD');
...
test('unattended control injects the official Full access policy at the gateway boundary', async(t)=>{
  ...
  assert.equal((await send('thread/settings/update',{threadId:'thread-1',collaborationMode:{mode:'plan',settings:{model:'gpt-5',reasoning_effort:'high'}}})).status,200,logs);
  assert.equal((await send('thread/settings/update',{threadId:'thread-1',collaborationMode:null})).status,200,logs);
  ...
  const planParams=readEntries().filter(item=>item.method==='thread/settings/update').at(-2)?.params,offParams=byMethod('thread/settings/update');assert.deepEqual(planParams.collaborationMode,{mode:'plan',settings:{model:'gpt-5',reasoning_effort:'high'}});assert.equal(offParams.collaborationMode,null);
