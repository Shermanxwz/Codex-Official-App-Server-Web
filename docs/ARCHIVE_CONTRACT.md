...
The human-facing Web surface includes official image input, an inventory summary for MCP servers/Skills/installed Apps, and an experimental-only view of the under-development official Plugin inventory. It also provides a collapsed technical view for method identifiers, schemas, parameters, and results. Image attachments are bounded and sent as the official `UserInput` `image` variant; arbitrary generic-file upload is not invented where the official protocol has no such input variant.
