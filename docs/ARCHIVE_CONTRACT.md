...
History is sealed as two explicit read paths: quick view requests one official `thread/turns/list` summary page containing the 10 newest Turns and never renders an older-page control; full-history view walks official cursors one page at a time. Full-history work-process items are fetched per Turn through official `thread/items/list` when the Turn approaches the viewport or the user explicitly requests them. The official plan card is a persistent bottom affordance; live plan updates preserve a mobile user’s explicit expanded state. The Web also performs a bounded read-only reconciliation while it owns a Turn, so loss of only `turn/completed` cannot leave the local running indicator or local write state permanently stale.
