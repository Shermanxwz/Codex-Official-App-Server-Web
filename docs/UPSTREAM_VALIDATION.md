...
The notification-routing seal separately proves that observability cannot leak into conversation rendering. The generated archive surface must include `item/commandExecution/terminalInteraction`; that method is required to resolve to `official-event-log-only`, while a synthetic future ServerNotification must resolve to the same timeline default and return `ignore` at the pre-fallback routing checkpoint. This guards the exact failure class where a process/transport notification carries `threadId`/`turnId`/`itemId` and would otherwise be mistaken for a work item.
...
A scheduled/push advisory job installs `@openai/codex@latest` and runs both stable and experimental protocol seals. It is intentionally non-blocking for the archived baseline: upstream future drift should be visible without retroactively invalidating a previously sealed, pinned deployment. New ordinary ServerNotification methods remain timeline-safe by default even before a human-facing disposition is added.
